...
background_image = dict(source=Image.open("images/eriksterck.png"), xref="paper", yref="paper", x=0.5, y=0.5, sizex=0.95, sizey=0.95, xanchor="center", yanchor="middle", opacity=0.04, layer="below", sizing="contain")
...
def generate_vStorage_overview_df(df_vPartition_filtered, df_vDisk_filtered, df_vmList_filtered, df_vSnapshot_filtered):
    ...
    vDisk_for_VMs_not_in_vPartition = pd.merge(df_vDisk_filtered[['VM Name','Capacity (GiB)','Power State','MOID']],df_vPartition_filtered[['MOID']],on='MOID', how='left', indicator=True).query("`_merge`=='left_only'").drop(columns=['_merge'], axis=1)
